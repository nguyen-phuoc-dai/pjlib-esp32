...
static int echo_client_thread(void *arg)
{
    ...
    rc = pj_sockaddr_in_init( &sockaddr, pj_cstr(&s, client->server),
    ...
    rc = pj_sock_connect(sock, &sockaddr, sizeof(sockaddr));
    if (rc != PJ_SUCCESS) {
    ...
    PJ_LOG(3,("", "...socket connected to %s:%d", 
		  pj_inet_ntop2(pj_AF_INET(), &sockaddr.sin_addr,
		  		addr, sizeof(addr)),
		  pj_ntohs(sockaddr.sin_port)));
